...
public final class AgentSqlRenderUtil {
    private AgentSqlRenderUtil() {
    }

    public static String render(BoundSql boundSql, Configuration configuration, AgentDialect dialect) {
    ...
    private static Object resolveNestedValue(MetaObject metaObject, String property) {
        if (metaObject == null || property == null || property.isEmpty()) {
            return null;
        }
        if (!metaObject.hasGetter(property)) {
            return null;
    ...
    private static String formatValue(Object v, AgentDialect dialect) {
        if (v == null) {
            return dialect.nullLiteral();
        }
        if (v instanceof String) {
            return dialect.escapeString((String) v);
        }
        if (v instanceof Character) {
            return dialect.escapeString(v.toString());
        }
        if (v instanceof Boolean) {
            return dialect.formatBoolean((Boolean) v);
        }
        if (v instanceof Integer || v instanceof Long || v instanceof Short || v instanceof Byte
                || v instanceof Double || v instanceof Float || v instanceof BigDecimal) {
        ...
        if (v instanceof LocalDate) {
            return dialect.formatDate((LocalDate) v);
        }
        if (v instanceof LocalDateTime) {
            return dialect.formatTimestamp((LocalDateTime) v);
        }
        if (v instanceof byte[]) {
            return dialect.formatBinary((byte[]) v);
        }
        return dialect.escapeString(v.toString());
    }
